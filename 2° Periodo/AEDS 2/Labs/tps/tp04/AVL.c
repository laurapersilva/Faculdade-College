...
ArvoreAVL *alocateArvoreAVL(void) {
...
void freeArvoreAVL(ArvoreAVL *arvore) {
...
int altura(Node *node) {
  ...
  int alturaEsq = altura(node->esq);
  int alturaDir = altura(node->dir);
...
Node *rotacaoEsquerda(Node *node) {
  ...
  node->dir = aux->esq;
  aux->esq = node;
  node->fator = fatorBalanceamento(node);
  aux->fator = fatorBalanceamento(aux);
  return aux;
}
...
Node *rotacaoDuplaEsquerda(Node *node) {
  node->dir = rotacaoDireita(node->dir);
  return rotacaoEsquerda(node);
}

Node *rotacaoDuplaDireita(Node *node) {
  node->esq = rotacaoEsquerda(node->esq);
  return rotacaoDireita(node);
}

// Funcao para inserir um pokemon na arvore AVL com base na chave name com as logicas de balanceamento
Node *inserirArvoreAVL(Node *node, pokemon *p) {
  if (node == NULL) {
    node = alocateNode();
    node->pokemon = p;
    return node;
  ...
  if (strcmp(getName(node->pokemon), getName(p)) > 0) {
    node->esq = inserirArvoreAVL(node->esq, p);
  } else {
    node->dir = inserirArvoreAVL(node->dir, p);
  }
...
// Funcao para pesquisar um pokemon na arvore AVL com base na chave name e retornar o caminho percorrido => raiz dir esq dir esq dir esq SIM
bool pesquisarArvoreAVL(Node *node, char *name) {
  if (node == NULL) {
    printf("NAO\n");
    return false;
  }
  ...
  comparacoes++;
  if (strcmp(getName(node->pokemon), name) == 0) {
    printf("SIM\n");
    return true;
  }
  ...
  if (strcmp(getName(node->pokemon), name) > 0) {
    printf("esq ");
    return pesquisarArvoreAVL(node->esq, name);
  } else {
    printf("dir ");
    return pesquisarArvoreAVL(node->dir, name);
  }
}

// Wrapper para a funcao de pesquisar um pokemon na arvore AVL
bool pesquisarArvoreAVLWrapper(Node *node, char *name) {
    printf("%s \n", name);
    printf("raiz ");
    return pesquisarArvoreAVL(node, name);
}
...
int main(void) {
  ...
  // Criar a arvore AVL
  ArvoreAVL *arvore = alocateArvoreAVL();
  ...
  while (strcmp(id, "FIM") != 0 && strcmp(id, "0") != 0) {
    ...
    if (poke != NULL) {
      arvore->raiz = inserirArvoreAVL(arvore->raiz, poke);
    }
    scanf("%s", id);
  }
  ...
  while (strcmp(name, "FIM") != 0) {
    ...
    pesquisarArvoreAVLWrapper(arvore->raiz, name);
    scanf("%s", name);
  }
