...
void ordenarPokemonsPorBolha(Pokemon pokemons[], int totalPokemons);
...
void ordenarPokemonsPorBolha(Pokemon pokemons[], int totalPokemons) {
    for (int i = 0; i < totalPokemons - 1; i++) {
        for (int j = 0; j < totalPokemons - 1 - i; j++) {
            comparacoes++; // Incrementa comparação
            if (pokemons[j].id > pokemons[j + 1].id) {
                ...
                pokemons[j + 1] = temp;
                movimentacoes += 3; // Contabiliza movimentações
            }
        }
    }
}
...
int main(void) {
    ...
    // Ordena os Pokémon encontrados pelo ID
    ordenarPokemonsPorBolha(encontrados, encontradosCount);

    // Incrementa movimentações
    movimentacoes += encontradosCount; // Exemplo: assume uma movimentação para cada Pokémon encontrado

    // Imprime os Pokémon na ordem de seus IDs
