...
int main(void) {
    ...
    // Ordena os Pokémon encontrados pelo nome
    qsort(encontrados, encontradosCount, sizeof(Pokemon), compararPokemons);

    // Incrementa movimentações
    movimentacoes += encontradosCount; // Exemplo: assume uma movimentação para cada Pokémon encontrado

    // Imprime os Pokémon na ordem alfabética
    for (int i = 0; i < encontradosCount; i++) {
        printPokemon(&encontrados[i]); // Imprime cada Pokémon
        // Adiciona nova linha apenas se não for o último Pokémon
        if (i < encontradosCount - 1) {
        }
    }
