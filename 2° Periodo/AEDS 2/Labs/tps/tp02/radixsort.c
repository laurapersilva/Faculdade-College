...
#define MAX_ABILITIES_LENGTH 200
...
typedef struct {
    ...
    char type1[80];
    char type2[80];
    char abilities[MAX_ABILITIES_LENGTH];
    double weight;
    double height;
    int captureRate;
    bool isLegendary;
...
void ordenarPokemons(Pokemon pokemons[], int totalPokemons);
...
void ordenarPokemons(Pokemon pokemons[], int totalPokemons) {
    for (int i = 0; i < totalPokemons - 1; i++) {
        for (int j = 0; j < totalPokemons - i - 1; j++) {
            // Comparar habilidades
            if (strcmp(pokemons[j].abilities, pokemons[j + 1].abilities) > 0 ||
                (strcmp(pokemons[j].abilities, pokemons[j + 1].abilities) == 0 &&
                 pokemons[j].generation > pokemons[j + 1].generation)) {
                // Trocar
...
int main(void) {
    ...
    // Ordena os Pokémon encontrados pela primeira habilidade e geração
    ordenarPokemons(encontrados, encontradosCount);

    // Imprime os Pokémon na ordem
